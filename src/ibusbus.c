                               ...
                               const gchar  *client_name)
{
    ...
    if (result != NULL) {
        ...
        /* try to create input context again with the returned fd */
        if (result != NULL) {
            GError *error = NULL;
...
static void
_create_input_context_async_step_four_done (GDBusConnection *connection,
                                            GAsyncResult    *res,
                                            GTask           *task)
{
    GError *error = NULL;
                                             ...
                                             GTask           *task)
{
    ...
    GVariant *variant = g_dbus_connection_call_with_unix_fd_list_finish
        (connection, &out_fd_list, res, &error);
    IBusBus *bus;
    GCancellable *cancellable;
    ...
    if (length == 1) {
        ...
        g_dbus_connection_call (bus->priv->connection,
                                ...
                                ibus_get_timeout (),
                                cancellable,
                                (GAsyncReadyCallback)_create_input_context_async_step_four_done,
                                task);
                                     ...
                                     gpointer            user_data)
{
     ...
     * directly, because we need four async steps for create an IBusInputContext.
     ...
     * 3. If failed in step one, try to call Connect in sandbox.
     * 4. Call CreateInputContext again with the returned fd.
     * 5. New local sandbox IBusInputContext proxy of the remote IC, and continue the step two.
     */
    g_dbus_connection_call (bus->priv->connection,
            IBUS_SERVICE_IBUS,
            IBUS_PATH_IBUS,
