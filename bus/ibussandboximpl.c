...
/**
 * _ibus_sandbox_create_input_context:
 *
 * Implement the "CreateInputContext" method call of the
 * org.freedesktop.sandbox.IBus interface.
 */
static void
_ibus_sandbox_create_input_context (BusIBusSandBoxImpl    *ibus_sandbox,
                                    GVariant              *parameters,
                                    GDBusMethodInvocation *invocation)
{
    ...
    /* create the input_context_connection, if not available. */
    BusConnection *input_context_connection =
        bus_connection_get_input_context_connection (connection);
    if (input_context_connection == NULL) {
        ...
        input_context_connection = bus_connection_new (conn);
        ...
        bus_connection_set_input_context_connection (connection, input_context_connection);
    }

    BusInputContext *context =
            bus_ibus_impl_create_input_context
        (bus_ibus_impl_get_default (), input_context_connection, client_name);

    if (context) {
        const gchar *path = ibus_service_get_object_path ((IBusService *) context);
        bus_connection_allow_path (input_context_connection, path);

        /* the format-string 'o' is for a D-Bus object path. */
        g_dbus_method_invocation_return_value_with_unix_fd_list (invocation, g_variant_new ("(o)", path), fds);
                                           ...
                                           GDBusMethodInvocation *invocation)
{
    ...
    static const struct {
        ...
        { "CreateInputContext",    _ibus_sandbox_create_input_context },
    };
